        ...
        public void EmitModelMethod(MethodInfo method)
        {
            ...
            foreach (ParameterInfo pi in parameters)
            {
                    ...
                    pi != contentBodyParameter)
                {
                    ...
                    {
                        if (Attribute(pi, typeof(AmqpFieldMappingAttribute)) is AmqpFieldMappingAttribute fieldMapping)
                        {
            ...
            foreach (MethodInfo method in asynchronousHandlers)
            {
                ...
                if (parameters.Length > 0)
                {
                    ...
                    foreach (ParameterInfo pi in parameters)
                    {
                        ...
                        {
                            Emit("            __impl.m_" + (!(Attribute(pi, typeof(AmqpFieldMappingAttribute)) is AmqpFieldMappingAttribute fieldMapping)
                                                          ? pi.Name
